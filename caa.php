<?php include "header.php";?>

		<div id = "container">

			<div class="scroll-gallery_container xrow">

				<div class="scroll-gallery_text">

					<div class="title">
						<div class="title-main">Event Posters
						</div>
						<div class="title-info">
							Canva, Photoshop<br>
							2017
						</div>
					</div>

					<div class="description">
						<article>
								<p><a href="https://www.exploreveg.org/">Compassionate Action for Animals</a>
									is a Twin Cities based animal advocacy organization
									that promotes plant-based lifestyles
									through community outreach and education.
								</p>
                <p>Throughout the year, CAA organizes multiple
									events throughout Minneapolis and St. Paul
									to create a welcoming and inclusive community of vegans.
								</p>
								<p>I helped create marketing materials for these events.
								</p>
						</article>
					</div>
				</div>


				<div class="scroll-gallery_art">
					<ul>

            <!--
            <li>
              <div class="art-description" id="tabah_all">
                <div class="title">
                  <div class="title-main">
                    Tabah
                  </div>
                  <div class="title-info">
                    Watercolor on paper, dimensions?<br>
                    2017
                  </div>
                </div>
                <p>
                  Tabah album jacket covers!
                </p>
              </div>
            </li>
            -->



            <li>
              <a class="toggle-description" id="chili-poster0">
                <img class="img_hover" src="img/caa/chili-poster-color.png" height="100%" border="0" />
              </a>
            </li>

						<li>
							<div class="art-description" id="chili-poster">
								<div class="title">
									<div class="title-main">
										Vegan Chili Poster
									</div>
									<div class="title-info">
										Canva<br>
										2017
									</div>
								</div>
								<p>For CAA's 8th Annual Vegan Chili Cookoff,
									I worked with CAA executive director
									Laura Matanah to create a poster
									optimized for both print and Facebook.
									The printed posters were pinned around Minneapolis and St. Paul
									to advertise for the event.
								</p>
							</div>
						</li>

            <li>
              <a class="toggle-description" id="chili-poster-fb0">
                <img class="img_hover" src="img/caa/chili-poster-fb2.png" height="100%" border="0" />
              </a>
            </li>

						<li>
							<div class="art-description" id="chili-poster-fb">
								<div class="title">
									<div class="title-main">
										Vegan Chili Poster
									</div>
									<div class="title-info">
										Canva<br>
										2017
									</div>
								</div>
								<p>Optimized for Facebook.
								</p>
							</div>
						</li>

						<li>
							<a class="toggle-description" id="vegweek0">
								<img class="img_hover" src="img/caa/vegweek.jpg" height="100%" border="0" />
							</a>
						</li>

						<li>
							<div class="art-description" id="vegweek">
								<div class="title">
									<div class="title-main">
										VegWeek Poster
									</div>
									<div class="title-info">
										Canva<br>
										2017
									</div>
								</div>
								<p>VegWeek is a nationwide campaign encouraging people to "pledge veg"
									for a week. CAA hosted different events throughout University of Minnesota's campus.
									To promote these events, I was asked to convey a large amount of information
									through a poster presented on television screen monitors.</p>
								<p>Using given content and descriptions, I integtrated
									CAA's theme font and color scheme,
									and organized the content into a hierarchical design
									that highlights different events in a readable manner.

								</p>
							</div>
						</li>


              <!--
							<li>
								<a class="toggle-description" id="wild0">
									<img class="img_hover" src="img/oil/surrealist1.jpg" height="100%" border="0" />
								</a>
							</li>
							<li>
								<div class="art-description" id="wild">
									<div class="title">
										<div class="title-main">
											Wild
										</div>
										<div class="title-info">
											Oil on canvas, 30"x20"<br>
											2015
										</div>
									</div>
									<p>
										<em>"Wild"</em> is a hybrid oil painting of
										Salvador Dali's "The Dream" and
										Henri Rousseau's "Sleeping Gypsy."
									</p>
								</div>
							</li>

							<li>
								<img class="img_hover" src="img/oil/surrealist2.jpg" height="100%" border="0" />
							</li>

							<li>
								<img class="img_hover" src="img/oil/surrealist3.jpg" height="100%" border="0" />
							</li>

							<li>
								<img class="img_hover" src="img/oil/surrealist4.jpg" height="100%" border="0" />
							</li>

							<li>
								<img class="img_hover" src="img/oil/surrealist5.jpg" height="100%" border="0" />
							</li>
            -->

					</ul>
				</div>
			</div>
		</div>

<?php include "footer.php"; ?>
